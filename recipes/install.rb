...
vs_is_installed = is_vs_installed?(version)

install_url = File.join(node['visualstudio']['source'], node['visualstudio'][version][edition]['filename'])
install_log_file = win_friendly_path(
  File.join(node['visualstudio'][version]['install_dir'], 'vsinstall.log'))

iso_extraction_dir = win_friendly_path(File.join(Chef::Config[:file_cache_path], version))
setup_exe_path = File.join(iso_extraction_dir, node['visualstudio'][version][edition]['installer_file'])
...
windows_package node['visualstudio'][version][edition]['package_name'] do
